...
- `POST /incidents/simulated`：生成七类确定性 Harness 场景（5 类基础 + 2 类 adversarial）并创建可诊断事件。
...
- `POST /evaluation-runs`：使用 `Idempotency-Key` 提交 B0（rule-based，确定性）或 B1（LLM，无 key 时降级到 rule-based）批量评测。
